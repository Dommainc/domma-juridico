...
import { Plus, Download, Upload, FileText, User, DollarSign, Briefcase } from 'lucide-react'
...
export default function TrabalhistaPage() {
  ...
  async function handleExport() {
    exportToExcel(processosFiltrados, 'processos_trabalhistas')
...
async function handleImport(e: React.ChangeEvent<HTMLInputElement>) {
  ...
  try {
    ...
    // Adicionar area: 'trabalhista' a cada item
    const processosComArea = data.map(item => ({ 
      ...item, 
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="header">
          <div>
            <h1 className="header-title">👷 Processos Trabalhistas</h1>
            <p className="header-subtitle">Gerenciar demandas trabalhistas</p>
          </div>
        ...
        <div className="card">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div className="md:col-span-2">
              <input
                type="text"
                placeholder="Buscar por empreendimento, autor, descrição..."
        ...
        <div className="card">
          {processosFiltrados.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-slate-500 text-sm mt-2">Clique em "Novo Processo" para adicionar</p>
            </div>
          ) : (
